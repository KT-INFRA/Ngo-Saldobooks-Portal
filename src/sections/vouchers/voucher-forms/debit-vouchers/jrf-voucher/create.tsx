...
import { useCreateJRFDebitVoucher, useGetJRFEmployeeList, useGetProjectList } from 'api/voucher';
...
export default function JRFFellowshipVoucher() {
  const { projects } = useGetProjectList();
  const { employees } = useGetJRFEmployeeList();

  const { createVoucher, isLoading: isCreatingVoucher } = useCreateJRFDebitVoucher(
    (response: any) => {
      if (response?.result) {
        // Success response
        openSnackbar({
    ...
    <>
      <Formik initialValues={initialValues} validationSchema={validationSchema} onSubmit={handleSubmit}>
        {(formikProps) => {
          ...
          return (
            <MainCard title={<VoucherCardTitle voucherType="Debit Voucher" titleText="Employee/JRF"></VoucherCardTitle>}>
                          ...
                          <FieldArray
                            ...
                            render={({ remove, push }) => {
                              const totalAmount = values.items.reduce((acc: number, item: any) => {
                                const getNetAmount = () => {
                                  const licenceFees = Number(item.licence_fees) || 0;
                                  const electricityFees = Number(item.electricity_fees) || 0;
                                  const waterFees = Number(item.water_fees) || 0;
                                  const fellowshipFees = Number(item.fellowship_fees) || 0;
                                  const charges = licenceFees + electricityFees + waterFees + fellowshipFees;
                                  const totalAmount = Number(item.amount) || 0;
                                  return totalAmount - charges;
                                };
                                return acc + getNetAmount();
                              }, 0);
                              return (
                                <>
                                    ...
                                    <Table sx={{ minWidth: 650, width: 'auto' }}>
                                      <TableHead>
                                        <TableRow>
                                          ...
                                          <TableCell>Licence Fees</TableCell>
                                          <TableCell>Electricity</TableCell>
                                          <Tooltip title="Water Charge">
                                            <TableCell>WC</TableCell>
                                          </Tooltip>
                                          <Tooltip title="Fellowship without Stipend">
                                            <TableCell>FWS</TableCell>
                                          </Tooltip>
                                          ...
                                          return (
                                            <TableRow key={item.id}>
                                              <VoucherItem
                                                ...
                                                licence_fees={item.licence_fees}
                                                electricity_fees={item.electricity_fees}
                                                water_fees={item.water_fees}
                                                fellowship_fees={item.fellowship_fees}
