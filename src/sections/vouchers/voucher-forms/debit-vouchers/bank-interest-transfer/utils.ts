...
export const thirdValidationSchema = (selectedTab = 0) =>
  Yup.object().shape({
    fundingAgency: Yup.number().when([], {
      is: () => selectedTab === 0,
      then: (schema) => schema.required('Funding Agency is required').min(1, 'Please select a valid Funding Agency'),
      otherwise: (schema) => schema.notRequired()
    }),
    ifgtbAccountId: Yup.number().when([], {
      is: () => selectedTab === 1,
      then: (schema) => schema.required('IFGTB Account is required').min(1, 'Please select a valid IFGTB Account'),
      otherwise: (schema) => schema.notRequired()
    })
  });
...
export const formateCreateBankVoucherPayload = async (values: InitialValues, selectedTab: number = 0) => {
