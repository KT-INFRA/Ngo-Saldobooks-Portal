...
import { useCreateDebitVoucherGeneral, useGetAccountHead, useGetOwnBankAccounts, useGetPaymentType, useGetProjectList } from 'api/voucher';
...
// import Step4 from 'sections/projects/add-project/Step4';
                        ...
                        <Grid container spacing={2} p={2}>
                          ...
                          <Grid item xs={12} sm={6}>
                            ...
                            <Grid container direction="column" spacing={1}>
                              <Grid item xs={12} md={12}>
                                ...
                                <Autocomplete
                                  ...
                                  onChange={(e, accountHead) => {
                                    setFieldValue('accountHeadId', accountHead?.value ?? '');
                                  }}
                                  isOptionEqualToValue={(option, value) => option.value === value.value}
                                  style={{ width: '100%' }}
                                  id="account_head_id"
                                  defaultValue={
                                    accountHeads.find(
                                      (accountHead: { label: string; value: number }) => accountHead.value === values.accountHeadId
                                    ) ?? null
                                  }
                                  ...
                                  renderInput={(params) => (
                                    <TextField
                                      ...
                                      error={touched.accountHeadId && Boolean(errors.accountHeadId)}
                                      helperText={touched.accountHeadId && errors.accountHeadId}
                                    />
                                  )}
                                />
                              </Grid>
                              ...
                              ].map((field: { id: number; label: string; field: string; type: string }) => {
                                const value = values[field.field as keyof typeof initialValues];
                                return (
                                  <Grid item xl={12} xs={12} md={12}>
                                    ...
                                    <TextField
                                      ...
                                      helperText={
                                        touched[field.field as keyof typeof initialValues] &&
                                        errors[field.field as keyof typeof initialValues]
                                      }
                                      fullWidth
                                    />
                                  </Grid>
