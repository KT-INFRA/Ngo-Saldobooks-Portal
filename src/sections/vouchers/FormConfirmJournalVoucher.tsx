...
import { useInsertJournalVoucherConfirmation } from 'api/voucher';
// assets
...
export default function FormVoucherConfirm({
  ...
  const { journalVoucherConfirm, isLoading: isCreatingVoucher } = useInsertJournalVoucherConfirmation(
    (response: any) => {
      if (response?.result) {
        // Success response
        openSnackbar({
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const { voucher_no: number, voucherDate: date } = voucherConfirmationDate;
        ...
        await journalVoucherConfirm(finalVoucherConfirmationData as any);
      } catch (error) {
        // console.error(error);
      }
    }
