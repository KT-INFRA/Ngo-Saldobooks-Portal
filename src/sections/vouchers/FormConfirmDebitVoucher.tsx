...
import { useInsertDebitVoucherConfirmation } from 'api/voucher';
// assets
...
export default function FormVoucherConfirm({
  ...
  const { debitVoucherConfirm, isLoading: isCreatingVoucher } = useInsertDebitVoucherConfirmation(
    (response: any) => {
      if (response?.result) {
        // Success response
        openSnackbar({
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        await debitVoucherConfirm(finalVoucherConfirmationData as any);
      } catch (error) {
        // console.error(error);
      }
    }
