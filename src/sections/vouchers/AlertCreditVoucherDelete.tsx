...
import { useDeleteVoucher } from 'api/voucher';
...
export default function AlertProjectDelete({ id, voucher_date, voucher_number, open, handleClose, submit }: Props) {
  ...
  const { deleteCreditVoucher, isLoading: isdeleteCreditVoucherLoading } = useDeleteVoucher(
    (response: any) => {
      if (response?.result) {
        openSnackbar({
          open: true,
  ...
  const deletehandler = async () => {
    ...
    await deleteCreditVoucher(deletedVoucherData);
  };
  // ----------------------------------------------
