...
import { useDeleteJournalVoucher } from 'api/voucher';
...
export default function AlertProjectDelete({ id, voucher_date, voucher_number, open, handleClose, submit }: Props) {
  ...
  const { deleteJournalVoucher, isLoading: isdeleteJournalVoucherLoading } = useDeleteJournalVoucher(
    (response: any) => {
      if (response?.result) {
        openSnackbar({
          open: true,
  ...
  const deletehandler = async () => {
    ...
    await deleteJournalVoucher(deletedVoucherData);
  };
  // ----------------------------------------------
