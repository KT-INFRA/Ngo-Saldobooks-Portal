...
export const inputlayouts1: FieldLayout[] = [
  ...
  { field: 'account_type_id', step: 1, type: 'select', enabled: true, label: 'Account Head Type', placeholder: 'Select Account Head Type' },  // Added donor type field
];
...
export const basicFieldSchema = Yup.object({
  ...
  account_type_id: Yup.string().required('Account Head Type is required'),
});
...
export const formateOwnBankAccountPayload = async (values: InitialFormValues) => {
  return {
    ...
    account_type_id: values?.account_type_id ? Number(values.account_type_id) : null,
  };
};
